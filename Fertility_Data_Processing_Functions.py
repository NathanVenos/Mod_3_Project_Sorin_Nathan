...
def three_way_welch_test_w_pop(series_list, groups_list, alpha):
    ...
    combos = list(combinations([0,1,2,3], r=2))
...
def point_estimate_distributions(sample_input, sub_samples_list, sub_samples_titles, replacement):
    """
    ...
    interval_start = np.percentile(a=point_estimates, q=2.5)
    interval_end = np.percentile(a=point_estimates, q=97.5)
